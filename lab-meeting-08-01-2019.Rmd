...
# Transfectant data
a2_dat <- read_rds("../HaCaT_june_2018/outputs/dat.RDS") %>% 
...
a2_dncb_mods <- read_rds("../HaCaT_june_2018/outputs/a2_dncb_mods.RDS")
...
rini_df <- read_rds("../HaCaT_june_2018/outputs/rini_df.RDS")
ker_df <- read_rds("../HaCaT_june_2018/outputs/ker_df.RDS")
...
class: inverse,center,top, hide-slide-number
background-image: url(img/tasmanian-devil.jpg)
background-position: center
background-size: 100%

# .content-box-neutral.white.bold[Tasmanian Devil proteomics]
...
class: hide-slide-number,bottom,right
background-image: url(img/denver-guilty-dog.jpg)
background-size: cover

# .black.bold[The dog ate <br> my homework]
...
# .content.top.left.black.bold[HaCaT cells]
...
background-image:url(img/pmhc-pathway.png)
background-size: contain

.footer-note.small.bold[based on: Sylvain Cardinaud, 2008]
...
class: inverse, center, middle
background-image: url(img/jelly-beans.jpg)
background-size: 100%

## .content-box-neutral.white.bold[Mass spectrometry proteomics]
...
```{r, echo=FALSE,cache=FALSE,out.width=430,out.height=430}
knitr::include_graphics("img/peptidome-analysis.png")
```
...
knitr::include_graphics("img/protein-turnover.png")
```
...
Sample prep
1. Cell culture
2. Purification of MHC I
3. HPLC separation of peptides
...
4. Detect the ions
MS/MS
5. Fragment the peptide
6. Analyse fragment mass
7. Detect fragment ions

Peptide identification
1. De novo tags
2. Database searches
3. PTM identification
4. Homology search
5. Statistical filtering

Bioinformatics and validation
1. Distributions, motifs, literature searching etc.
---
...
.column.inverse[.content.center.vmiddle[
# How do we know we have MHC I peptides?
]]

.column.inverse[.content.center.vmiddle.img-contain[
![](img/middle_finger_gif.gif) 
]]
...
.horizon-1[Large variance observed in DNCB treated cells:] <br> 
Indication of DNCB toxicity at 10 $\mu$M?
  ]]
]]
...
```{r cell-viability,echo=FALSE,fig.retina=6,fig.asp=0.8,out.width="90%",cache=TRUE,warning=FALSE}
cell_viability %>% 
...
layout: true
class: inverse
background-image: url(img/dog-paper-ripped-up.jpg)
background-size: 100%

.split-four[
  .row.bg-main1[.content[
# .bold[Central questions] 
  ]]
  
  .row.bg-main2[.content.content-box-marine[
### .horizon-1[Are the modified peptides real and immunogenic?] <br>

+ Peptides have been synthesised. 
+ Peptides need to be modified and have spectra checked.
+ ELISPOT test on patient samples.


### .horizon-1[What are sensitisers doing to the proteome?] <br>
 
+ Protein turnover assays for DNCB and DPCP treated HaCaT A2 performed, but not processed.
]]
